...
const categoryColors: Record<string, string> = {
  Frontend: "bg-amber-400/15 text-amber-300 border-amber-400/25",
  Backend: "bg-blue-400/15 text-blue-300 border-blue-400/25",
      ...
      >
        ...
        <motion.div variants={item} className="lg:col-span-2 space-y-6">
          ...
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-br from-amber-500 to-orange-600 opacity-30 blur-md group-hover:opacity-50 transition-opacity duration-300" />
            <div className="plate-ticks relative overflow-hidden aspect-[4/3]">
              <img
                src="2.jpeg"
                alt="Shashwat Vaish"
            ...
            <div className="flex gap-2">
              {[
                ...
                <a
                  ...
                  aria-label={label}
                  className="flex-1 flex items-center justify-center h-10 bg-white/5 border border-white/10 text-foreground/50 hover:text-amber-400 hover:border-amber-400/30 transition-all duration-200"
                >
          ...
          <div className="plate-ticks glass-dark p-7 gradient-border">
            ...
            <div className="space-y-4 text-foreground/65 leading-relaxed">
              <p>
                ...
                <span className="text-amber-400 font-medium">GGS Indraprastha University</span>, who enjoys building products that combine clean design with solid engineering. What started as curiosity during school gradually turned into a strong passion for creating scalable, real-world applications.
              </p>
              <p>
              ...
              {[
                ...
                <div key={edu.degree} className="flex gap-4">
                  <div className="flex flex-col items-center gap-1 pt-1">
                    <div className={`h-3 w-3 rounded-full ${edu.dot} ring-2 ring-offset-2 ring-offset-[#0d0c0a] ring-white/10`} />
                    <div className="w-px flex-1 bg-white/10" />
                  </div>
                  <div className="pb-4">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <h3 className="font-semibold text-foreground/90">{edu.degree}</h3>
                      <span className={`font-mono text-[10px] uppercase tracking-wide px-2 py-0.5 ${edu.status === 'Current' ? 'bg-amber-400/15 text-amber-300' : 'bg-blue-500/15 text-blue-300'}`}>
                        {edu.status}
                      </span>
                    </div>
          ...
          <div className="plate-ticks glass-dark p-7 gradient-border">
            ...
            <div className="flex flex-wrap gap-2">
              {interests.map((interest) => (
                <span
                  key={interest}
                  className="px-4 py-2 text-sm font-medium bg-white/5 text-foreground/60 border border-white/10 hover:border-amber-400/30 hover:text-foreground/80 transition-all duration-200 cursor-default"
                >
                  {interest}
                </span>
              ))}
