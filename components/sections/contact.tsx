...
import { Send, Mail, MapPin, Github, Linkedin, Instagram, ArrowRight, CheckCircle2 } from 'lucide-react';
...
export default function Contact() {
  ...
  const infoItems = [
    { icon: Mail, label: "Email", value: "shashwatvaish1@gmail.com", href: "mailto:shashwatvaish1@gmail.com", colorClass: "text-amber-400 bg-amber-400/15" },
  ...
  const inputCls = "w-full px-4 py-3 bg-white/5 border border-white/10 text-foreground placeholder-foreground/30 focus:outline-none focus:border-amber-400/50 focus:ring-1 focus:ring-amber-400/30 transition-all duration-200 text-sm";
          ...
          <div className="plate-ticks glass-dark p-7 gradient-border h-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <motion.button type="submit" disabled={isLoading || formStatus === 'success'} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                className="w-full py-3 px-6 font-semibold text-sm text-[#1a1408] bg-amber-400 hover:bg-amber-300 disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2 shimmer">
                {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="lg:col-span-2 space-y-5">
          <div className="plate-ticks glass-dark p-6 gradient-border space-y-4">
            ...
            {infoItems.map(({ icon: Icon, label, value, href, colorClass }) => (
              <div key={label} className="flex items-center gap-4 p-4 bg-white/3 border border-white/8 hover:border-amber-400/20 transition-all duration-200">
                ...
                <div className="min-w-0">
                  ...
                  {href ? (
                    <a href={href} className="text-sm font-medium text-foreground/80 hover:text-amber-300 transition-colors truncate block">{value}</a>
                  ) : (
            ...
            <div className="space-y-3">
              {socialLinks.map(({ href, icon: Icon, label }) => (
                <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 p-3.5 glass border border-white/8 text-foreground/50 hover:text-foreground/90 hover:border-amber-400/30 transition-all duration-200"
                  whileHover={{ x: 4 }}>
          ...
          <div className="p-5 border border-amber-400/20 bg-amber-400/5">
            <p className="text-xs text-foreground/50 leading-relaxed">
              I typically respond within <span className="text-amber-300 font-medium">24 hours</span>. Whether it's a collaboration, internship, or just a chat — I'm all ears!
            </p>
          </div>
        </motion.div>
      </div>
